  ...
  if (resumeObject.basics.profiles) {
    if (resumeObject.basics.profiles[0].network) {
      _.each(resumeObject.basics.profiles, function(w){
        w.class = "fa fa-" + w.network.toLowerCase();
        if (w.network.toLowerCase() == 'twitter') {
					if (w.url === '' && w.username !== '') { w.url = "https://twitter.com/" + w.username; }
				}
        if (w.network.toLowerCase() == 'facebook') {
					if (w.url === '' && w.username !== '') { w.url = "https://facebook.com/" + w.username; }
				}
        if (w.network.toLowerCase() == 'linkedin') {
					if (w.url === '' && w.username !== '') { w.url = "https://linkedin.com/in/" + w.username; }
				}
				if (w.network.toLowerCase() == 'github') {
					if (w.url === '' && w.username !== '') { w.url = "https://github.com/" + w.username; }
				}
        if (w.network.toLowerCase() == 'stackoverflow') {
					w.class = "fa fa-stack-overflow";
        }
      });
    }
  }
